...
def loadDictionary():
	...
	for letter in tilebag.TurkishCharList:
	    AIsearchWord[(index,letter)] = []
    # #############################################################	   
    
     #load meaning list
    with codecs.open("data/db/meaninglist.txt", 'r', 'utf-8') as openfile:
	for line in openfile.read().splitlines():
	    (word,meaning) = line.split("**")
	    
	    # New: AI map with index of char and char to word list
	    for ch in word:
		if ch in tilebag.TurkishCharList:
		    AIsearchWord[(word.index(ch)+1,unicode(ch.lower()))].append((word, len(word)))
	    # ###################################################
	    
	    if meaning == 'yok':
		DICTIONARY[word] = "Kelime anlami bulunamadi."
	    else:
		DICTIONARY[word] = meaning
    openfile.close()  
    #load word list
    with codecs.open("data/db/wordlist.txt", 'r', 'utf-8') as openfile:
...
def aiOperations():
	...
	for y in range(15):
		...
		tkMessageBox.showinfo(title="CurLetters after rightlist", message=currLetters)   
